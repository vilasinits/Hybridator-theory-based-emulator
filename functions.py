...
def inverse_cdf_transform(image, cdf_target, bin_target, nbins):
    ...
    source_bins = jnp.linspace(bin_centers[0], bin_centers[-1]+(bin_centers[1]-bin_centers[0]), 5000)
    ...
    target_bins = jnp.linspace(bin_target[0], bin_target[-1], 5000)
...
def get_target_values(data: np.ndarray, angle: u.Quantity, ell_min: int = 0, ell_max: int = 24000, cls_bins: int = 50, nscales: int = 1, pdf_bins: np.ndarray = np.array([200]), filter="tophat"):
    ...
    if filter=="starlet":
        target_wavelet = starlet_decompose(target, nscales)        
    else:
        target_wavelet = tophatdecompose(target, nscales)
     
    # target_wavelet = jnp.array(target_wavelet)    
    target_edges, target_bin_centers, target_pdf = calculate_pdf(target, pdf_bins[0])
    target_cdf = calculate_cdf(target_pdf, target_edges) 
    
    starlet_results = process_starlet_scales(target_wavelet, angle_deg, ell_min, ell_max, cls_bins, pdf_bins, nscales)
...
def process_starlet_scales(target_starlet: np.ndarray, angle: float, ell_min: int, ell_max: int, cls_bins: int, pdf_bins: np.ndarray, nscales: int):
    starlet_edges = []
    starlet_bin_centers = []
    starlet_pdf = []
    starlet_cdf = []
    ...
    for i in range(nscales + 1):
        edges, bin_centers, counts = calculate_pdf(target_starlet[i], pdf_bins[i+1])
