...
def hybridate(redshift, file_number, model_name="model_one",ell_min=0, ell_max=24000, cls_bins=50, nscales=1, pdf_bins: np.ndarray = np.array([200,200]), no_iterations=50, plot=False, filter="tophat"):
    # data, angle_ = load_data_mnu(redshift, file_number, model_name)
    data, angle_ = load_data(file_no=file_number)
    target_results = get_target_values(data, angle_, ell_min, ell_max, cls_bins, nscales, pdf_bins, filter)
    ...
    for i in range(no_iterations):
        print(f"Emulation Iteration: {i}", end='\r')
        corrected_cls_map = generate_field_with_target_cls(initial_map, angle, target_cls_values, target_ell_bins, ell_max, cls_bins)
        if filter=="starlet":
            initial_wavelet = starlet_decompose(initial_map, nscales)        
        else:
            initial_wavelet = tophatdecompose(initial_map, nscales)
        corrected_pdf_starlet_map = []
        for scale in range(len(target_starlet)):
            corrected_pdf_starlet_map.append(inverse_cdf_transform(initial_wavelet[scale], target_starlet_cdf[scale], target_starlet_bin_centers[scale], pdf_bins[scale+1]))
        ...
        if plot:
            ...
            if filter=="starlet":
                corrected_wavelet = starlet2d(corrected_full_map, nscales)
            else:
